...
from fosslib.books.models import Book
from fosslib.utils import flash_form_errors
...
blueprint = Blueprint(
    "frappe_api",
    __name__,
    url_prefix="/import_books",
)


@blueprint.route("/frappe/", methods=("GET", "POST"))
async def import_books():
    ...
    if form.validate_on_submit():
        await fetch_books_from_form(form)

    flash_form_errors(form)
    return render_template("import_books/frappe.html", form=form)


async def fetch_books_from_form(form):
    """Fetches the books from the farm and flashes the appropriate messages if any"""
