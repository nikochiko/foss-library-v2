...
from fosslib.utils import get_cover_image_by_isbn
...
async def get_n_books_from_api(
    ...
    async with aiohttp.ClientSession() as session:
        async for json_book in fetch_n_pages_concurrently(session, n_pages, params):
            ...
            book_obj = await convert_json_to_book(session, json_book)
            sent_count += 1
            yield book_obj
...
async def convert_json_to_book(
    session: aiohttp.ClientSession, json_book: dict[str, Any]
) -> Book:
    ...
    kwargs["cover_image"] = await get_cover_image_by_isbn(session, kwargs["isbn13"])
